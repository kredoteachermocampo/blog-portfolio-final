            ...
            <form method="post">
                ...
                <div class="input-group">
                    ...
                    <select name="author_id" class= "form-select border-3 border-dark border-top-0 border-end-0 border-start-0 rounded-0">
                        <?php
                            $users_result = getUser();
                            while($users_row = $users_result->fetch_assoc()){
                                if($users_row['account_id'] == $posts_result['account_id']){
                        ?>
                                <option value="<?= $users_row['account_id'] ?>" selected>
                                    <?= $users_row['username'] ?>
                                </option>
                        <?php
                                }else{
                        ?>
                                <option value="<?= $users_row['account_id'] ?>">
                                    <?= $users_row['username'] ?>
                                </option>    
                        <?php
                                }
